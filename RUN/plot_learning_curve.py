...
def plot_learning_curve(estimator, title, X, y, axes=None, ylim=None, cv=ShuffleSplit(n_splits=5, test_size=0.2, random_state=0),
    ...
    axes[0].set_title("Learning Curve: "+title)
    if ylim is not None:
    ...
    # plt.title(str(title),y=0.0)

    #plt.show()
    fig.tight_layout()
    plt.savefig('plt_lrncrv_'+str(title))

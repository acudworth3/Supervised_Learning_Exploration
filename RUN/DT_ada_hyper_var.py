...
#     DT_ab = DecisionTreeClassifier(ccp_alpha=alpha)
    DT_ab = AdaBoostClassifier(DecisionTreeClassifier(class_weight={0:(1 - alpha)/2, 1: (1 - alpha)/2,2:(1 - alpha)/2}))
    ...
    DT_pkr = AdaBoostClassifier(DecisionTreeClassifier(class_weight={0:1-alpha, 1:alpha}))
...
plt.ylabel('Accuracy (%)')
plt.title('Boosted DT Performance vs Poker Target Weight')
plt.legend(loc='best')
# plt.xticks(list(ab_train_scores.keys()))
# plt.show()
plt.savefig('DT_boosted_weight_pkr'+str(max(pkr_test_auc.values()))+'.png')
plt.close()

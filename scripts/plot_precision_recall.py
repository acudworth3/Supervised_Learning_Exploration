...
def plot_precision_recall(data_obj,clf):
    ...
    classifier = clf
    n_classes = int(max(data_obj.all[data_obj.target].values)+1)
    ...
    for i, color in zip(range(n_classes), colors):
        ...
        lines.append(l)
        labels.append('p-r class {0} (area = {1:0.2f})'
    ...
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall curve '+str(data_obj.title))
    plt.legend(lines, labels, prop=dict(size=7))


    # plt.show()
    plt.savefig('plt_precrec_'+str(data_obj.title))